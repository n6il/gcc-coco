...
relr()
{
	...
	addr_t r, rtbase, rtofst, pc;
	addr_t paga = 0, pags = 0;  /* to fix a warning */
	struct areax **a;
	struct sym **s;
	...
	/*
	 * Check that we belong to the reloc area
	 */
	if (a[aindex]->a_bap != reloc_ap)
		return;
	...
	while (more()) {
		...
		if (mode & R_SYM) {
			if (rindex >= hp->h_nglob) {
				fprintf(stderr, "R symbol error (reloc)\n");
				return;
			}
			reli = symval(s[rindex]);
		} else {
			if (rindex >= hp->h_narea) {
				fprintf(stderr, "R area error (reloc)\n");
				return;
			}
...
relp()
{
	...
	while (more()) {
		...
		if (mode & R_SYM) {
			if (rindex >= hp->h_nglob) {
				fprintf(stderr, "P symbol error (reloc)\n");
				return;
			}
			relv = symval(s[rindex]);
		} else {
			if (rindex >= hp->h_narea) {
				fprintf(stderr, "P area error (reloc)\n");
				return;
			}
	...
	aindex = add_w(0,2);
	if (aindex >= hp->h_narea) {
		fprintf(stderr, "P area error (paged)\n");
		return;
	}
